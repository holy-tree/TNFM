...
class Dataset(BaseDataset):
    ...
    def __getitem__(self, i):
       ...
       # image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        ...
        # img = image.clone().detach().double().to(torch.device('cpu'))
        # # img = np.ascontiguousarray(image.numpy().transpose((1,2,0)))
        # print(self.images_fps[i])
        # cv2.imwrite(f"img_ori.jpg", image)

        # # img = mask.clone().detach().double().to(torch.device('cpu'))
        # # img = np.ascontiguousarray(mask.numpy())
        # print(self.masks_fps[i])
        # cv2.imwrite(f"img_mask.jpg", mask)
        # quit()
