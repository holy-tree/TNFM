...
class HYBNet(nn.Module):
    def __init__(self, 
                 vit: nn.Module,
                 resnet: nn.Module,
                 is_sam: bool,
                 catn: bool):
        super(HYBNet, self).__init__()
        ...
        self.fc1 = nn.Conv2d(in_channels=96, out_channels=768, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc2 = nn.Conv2d(in_channels=192, out_channels=768, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc3 = nn.Conv2d(in_channels=384, out_channels=768, kernel_size=1, stride=1, padding=0, bias=True)
        # self.fc4 = nn.Conv2d(in_channels=768, out_channels=768, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.ffc1 = nn.Conv2d(in_channels=768, out_channels=96, kernel_size=1, stride=1, padding=0, bias=True)
        self.ffc2 = nn.Conv2d(in_channels=768, out_channels=192, kernel_size=1, stride=1, padding=0, bias=True)
        self.ffc3 = nn.Conv2d(in_channels=768, out_channels=384, kernel_size=1, stride=1, padding=0, bias=True)
        # self.ffc4 = nn.Conv2d(in_channels=768, out_channels=768, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        if self.catn:
            ...
            hyb1 = self.catn1(c1, vit_x[0])
            hyb2 = self.catn2(c2, vit_x[1])
            hyb3 = self.catn3(c3, vit_x[2])
            # hyb4 = self.catn4(c4, vit_x[3])
            ...
            # hyb4 = self.norm4(hyb4).transpose(1, 2).reshape(bs, dim, H4, W4)
