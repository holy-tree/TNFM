...
import sys
sys.path.append('..')
from backbone.vit import PatchEmbed, Block
...
class MAEVit(nn.Module):
    def __init__(self, 
        ...
        self.blocks = nn.Sequential(*[
            Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio,
                  ...
                  drop_path=dpr[i], norm_layer=norm_layer, act_layer=nn.GELU,
                  windowed=False, 
                  layer_scale=True, with_cp=False,
                  use_residual= False) for i in range(depth)
        ])
        ...
        self.decoder_blocks = nn.ModuleList([
                Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, 
                      ...
                      windowed=False, 
                      layer_scale=True, with_cp=False,
                      use_residual= False) for i in range(decoder_depth)
        ])
    ...
    def _get_pos_embed(self, pos_embed, H, W):
        pretrain_size = [224, 224]
        pos_embed = pos_embed.reshape(1, pretrain_size[0] // 16, pretrain_size[1] // 16, -1).permute(0, 3, 1, 2)
